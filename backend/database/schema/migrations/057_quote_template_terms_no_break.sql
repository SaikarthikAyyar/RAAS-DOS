-- Direct follow-up: the Commercial Proposal section's closing line
-- ("We look forward to your valuable orders.") was landing alone on
-- its own page because "TERMS & CONDITIONS" forced its own page break
-- right after it, wasting most of that page. "TERMS & CONDITIONS" is
-- prefixed with "^" (the same opt-out convention already used on
-- "TANK / MACHINE DETAILS") so it now flows directly onto whatever
-- page the Commercial Proposal content ends on instead of starting a
-- fresh one.
