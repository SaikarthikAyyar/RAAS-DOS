-- Direct follow-up per the user's latest page-layout instructions:
--   - Introduction gets its own full page (unchanged - it's already a
--     normal heading, which now defaults to forcing a page break under
--     the new add_section_heading() mechanism in quote_release_docx.py).
--   - Project Overview and Tank/Machine Details now share a page:
--     "TANK / MACHINE DETAILS" is prefixed with "^", the new opt-out
--     convention that skips the forced page break so it stays on the
--     same page as the immediately-preceding Project Overview section.
--   - The static "Machine Overview:" label line before {machine_features}
--     is removed - the machine section's heading is now auto-generated
--     dynamically by build_machine_section() using the resolved
--     machine's real name, so a second static label immediately above
--     it would be redundant.
--   - "COMMERCIAL PROPOSAL" is left as a normal (non "^") heading, so it
--     still forces its own new page as required.
